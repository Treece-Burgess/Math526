...
u_func=@(z)(abs(z)<40).*(-15+0.01.*z.^2)+(abs(z)>=40).*(z.*0);%(abs(z)<40).*(-15*exp(-(1/1000)*z.^2))+(abs(z)>=40).*(z.*0); %mean(y)*z.^0;% %(abs(z)<40).*(-17*cos((z.*pi)./80))+(abs(z)>=40).*(0); 
lambda=1;
l=5;
n=101;
s=10000;                         
...
cdf_5=mvncdf([-20,-20,-20,-20,-20], u_alpha, sigma_alpha);
for i=1:s
    mc_5=zeros(s,1);
